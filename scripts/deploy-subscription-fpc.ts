...
import { SubscriptionFPCContract, SubscriptionFPCContractArtifact } from '@gregojuice/contracts/artifacts/SubscriptionFPC';
...
async function main() {
  ...
  const { fpcAddress, secretKey } = await deployAndRegisterSubscriptionFpc(node, wallet, fpcDeployer, paymentMethod);
  ...
  const signedUpFunctions = await executeFpcSignUps(fpcAddress, fpcDeployer, wallet, paymentMethod, config.contracts);
...
async function claimFeeJuiceOnL2(node: AztecNode, feeJuiceClaim, wallet: EmbeddedWallet, fpcAddress: AztecAddress, fpcDeployer: AztecAddress, paymentMethod: SponsoredFeePaymentMethod) {
  // Wait for the L1->L2 bridge message and claim the FJ to credit the FPC's balance.
  console.log('\nWaiting for L1->L2 message sync...');
  await waitForL1ToL2MessageReady(node, Fr.fromHexString(feeJuiceClaim.messageHash), { timeoutSeconds: 120 });
  console.log('Message ready');

  console.log('Claiming fee juice on L2 for FPC...');
  await FeeJuiceContract.at(wallet).methods
    .claim(fpcAddress, feeJuiceClaim.claimAmount, feeJuiceClaim.claimSecret, feeJuiceClaim.messageLeafIndex)
...
function updateNetworkConfigFile(config: any, fpcAddress: AztecAddress, secretKey: Fr, signedUpFunctions: ResolvedSignup[], configPath: string) {
...
async function bridgeTokens(node: AztecNode, l1RpcUrl: string, l1FunderKey: string, fpcAddress: AztecAddress) {
...
async function executeFpcSignUps(
  ...
  for (const { addressKey, contractAddress, functionName, selector, maxUses, maxFee, maxUsers } of functionsToSignupToFpc) {
...
async function resolveFpcSignups(
  ...
  return Promise.all(
    specs.flatMap(spec => {
      ...
      if (!fn) {
        throw new Error(`Function ${spec.functionName} not found in artifact ${spec.artifact.name}`);
      }
      ...
      return spec.contractAlias.map(async addressKey => {
        ...
        return { addressKey, contractAddress, functionName: spec.functionName, selector, maxUses, maxFee, maxUsers };
      });
    }),
  );
}
...
async function deployAndRegisterSubscriptionFpc(node: AztecNode, wallet: EmbeddedWallet, deployer: AztecAddress, paymentMethod: SponsoredFeePaymentMethod) {
  const { address: fpcAddress, secretKey } = await deploySubscriptionFpc(wallet, deployer, paymentMethod);
