...
export function EmojiVerification({ wallet, pendingConnection, onConfirm, onCancel }: EmojiVerificationProps) {
  return (
    <>
      <Box
        sx={{
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
          {wallet.icon ? (
            <Box component="img" src={wallet.icon} alt={wallet.name} sx={{ width: 40, height: 40, borderRadius: 1 }} />
          ) : (
            <Box
              sx={{
                width: 40,
      ...
      >
        ...
        <Typography variant="caption" color="text.secondary">
          Verify the emoji code above matches what your wallet is showing. If they don't match, click "Cancel" - someone
          may be trying to intercept your connection.
        </Typography>
      </Box>
