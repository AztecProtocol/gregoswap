    ...
    for (const group of groups) {
      ...
      if (allLeafLike) {
        ...
        for (const child of group.nodes) {
          ...
          laneEnds[lane] = end;
          items.push({
            record: child.record, nodeId: child.id,
            depth: child.depth, row: baseRow + lane, hasChildren: false,
          });
        }
        ...
        for (const child of group.nodes) {
          if (isLeafLike(child)) {
            items.push({
              record: child.record, nodeId: child.id,
              depth: child.depth, row: nextRow, hasChildren: false,
            });
            nextRow++;
          } else {
            visitNode(child);
...
function WaterfallChart({ report, minDuration }: { report: ProfileReport; minDuration: number }) {
  ...
  const dragRef = useRef<{
    ...
    scrollLeft0: number; scrollTop0: number;
    dragStartX: number; // canvas-space x where drag started
    shiftKey: boolean; moved: boolean;
  } | null>(null);
  ...
  const doZoom = useCallback((nextZoom: number) => {
    // Preserve the center of the current viewport.
    const centerRatio = virtualContentW > 0
      ? (scrollLeft + chartViewportW / 2) / virtualContentW
      : 0.5;
  ...
  const toVirtualX = useCallback((e: React.MouseEvent): number => {
    const canvas = chartCanvasRef.current;
    if (!canvas) return 0;
  ...
  const chartHitTest = useCallback((e: React.MouseEvent): LayoutItem | null => {
    const canvas = chartCanvasRef.current;
  ...
  const labelHitTest = useCallback((e: React.MouseEvent): LayoutItem | null => {
    const canvas = labelCanvasRef.current;
    if (!canvas) return null;
    const rect = canvas.getBoundingClientRect();
    const canvasY = e.clientY - rect.top;
  ...
  const onLabelMouseMove = useCallback((e: React.MouseEvent) => {
    const canvas = labelCanvasRef.current;
    if (!canvas) return;
    const item = labelHitTest(e);
  ...
  const onChartMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (dragRef.current) {
      ...
      if (dragRef.current.shiftKey) {
        ...
        const newScrollLeft = Math.max(0, Math.min(maxScrollLeft, dragRef.current.scrollLeft0 - dx));
  ...
  const onChartMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    <Box ref={outerRef} sx={{ fontFamily: 'monospace', userSelect: 'none' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
        <Typography sx={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', flexGrow: 1 }}>
          ...
          {zoom > 1 ? ` \u00b7 ${zoom}x zoom` : ''}
          &nbsp;&middot; drag to pan &middot; wheel to scroll &middot; ctrl+wheel or shift+drag to zoom
        </Typography>
        <Button size="small" variant="text" sx={btnSx}
          disabled={zoom <= 1}
        ...
        {expanded.size < expandableIds.size && expandableIds.size > 0 && (
          <Button size="small" variant="text" sx={btnSx} onClick={() => setExpanded(new Set(expandableIds))}>
            expand all
          </Button>
        )}
      </Box>
...
function SummaryTable({ report, minDuration }: { report: ProfileReport; minDuration: number }) {
  ...
  const td = { fontSize: 10, color: 'rgba(255,255,255,0.6)', fontFamily: 'monospace', py: 0.25, px: 1 };
  const tdR = { ...td, textAlign: 'right' as const };
  const tdDim = { ...tdR, color: 'rgba(255,255,255,0.35)' };

  const AggTable = ({ rows, color, max = 15 }: {
    ...
    <Box component="table" sx={{ borderCollapse: 'collapse' }}>
      <tbody>
        {rows.slice(0, max).map(([name, { count, totalMs, maxMs }]) => (
          <tr key={name}>
            <Box component="td" sx={{ ...td, color: color ?? td.color, maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</Box>
            <Box component="td" sx={tdR}>&times;{count}</Box>
            <Box component="td" sx={tdR}>{fmt(totalMs)}</Box>
            <Box component="td" sx={tdDim}>max {fmt(maxMs)}</Box>
          </tr>
        ))}
      </tbody>
    </Box>
          ...
          <tbody>
            {([...byCat.entries()] as [Category, { count: number; totalMs: number }][])
              ...
              .map(([cat, { count, totalMs }]) => (
                <tr key={cat}>
                  <Box component="td" sx={{ ...td, color: CATEGORY_COLORS[cat], fontWeight: 700 }}>{cat}</Box>
                  <Box component="td" sx={tdR}>{count}</Box>
                  <Box component="td" sx={tdR}>{fmt(totalMs)}</Box>
                </tr>
              ))}
          </tbody>
  ...
  return createPortal(
    <Box sx={{
      ...
      <Box sx={{
        ...
        <Slider size="small" min={0} max={50} step={0.5} value={minDuration}
  ...
  return (
    <>
      <Paper elevation={8} sx={{
        ...
        {report && !recording && (
          <>
            <Chip label={fmt(report.durationMs)} size="small"
